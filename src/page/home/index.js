import {Button, Col, Input, notification, Row, Spin, Modal} from "antd";
...
const HomePage = () => {
  ...
  const showConfirm = () => {
    confirm({
      centered:true,
      title: `Do you want to send these ${data.length} emails?`,
      icon: <ExclamationCircleOutlined/>,
      content: 'Some descriptions',
      onOk: async () => {
        setLoading(true)
        await sendMailAuto({
          value: await encodeJwt({
            BodyHtml: template,
            Subject: subject,
            Emails: data
          })
                  ...
                  <Col xl={12} className={""}>
                    ...
                    <div className={"mt-2 d-flex justify-content-center"}>
                      <div>Number of emails to send:
                        {loadingFile ?
                          <div className={"d-flex justify-content-center"}><Spin tip="Loading..." className={"mt-2"}
                                                                                 spinning={loadingFile} size="large"/>
                          </div> :
